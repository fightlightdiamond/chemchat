...
check_prerequisites() {
    ...
    if ! command_exists npm; then
        print_warning "npm is not installed. Installing globally..."
        npm install -g npm
    fi
...
setup_database() {
    ...
    docker-compose exec chemchat-app npm exec prisma migrate deploy
    
    # Generate Prisma client
    print_status "Generating Prisma client..."
    docker-compose exec chemchat-app npm exec prisma generate
    
    # Seed database with test data
    print_status "Seeding database with test data..."
    docker-compose exec chemchat-app npm exec ts-node scripts/basic-seed.ts
...
show_service_urls() {
    ...
    echo "🛠️  Run migrations:        docker-compose exec chemchat-app npm exec prisma migrate dev"
    echo "🌱 Seed database:         docker-compose exec chemchat-app npm exec ts-node scripts/basic-seed.ts"
...
main() {
    case "${1:-start}" in
        ...
        "seed")
            ...
            docker-compose exec chemchat-app npm exec ts-node scripts/basic-seed.ts $2 $3 $4 $5
            ;;
        "migrate")
            print_status "Running database migrations..."
            docker-compose exec chemchat-app npm exec prisma migrate dev
            ;;
        "shell")
