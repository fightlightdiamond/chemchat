...
export function setup() {
  ...
  const tenantResponse = http.post(`${BASE_URL}/tenants`, JSON.stringify(tenantPayload), {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const userResponse = http.post(`${BASE_URL}/auth/register`, JSON.stringify(userPayload), {
      headers: {
...
export function httpLoadTest(data) {
  ...
  const conversationsResponse = http.get(`${BASE_URL}/chat/conversations`, { headers });
